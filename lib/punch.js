	...
	report: function() {
		...
		for (day in this.days) {
			...
			if (this.days.hasOwnProperty(day)) {
				...
				report += day.bold.red + ": " + summary.duration.precise().bold.blue + "\n";
				for (task in summary.tasks) {
					if (summary.tasks.hasOwnProperty(task)) {
						report += "  " + task.green + ": " + summary.tasks[task].precise().blue + "\n";
					}
				}
...
punch.status = punchCommand(function(options){
	...
	if (task) {
		if (task.isActive()) {
			console.log(task.what.green + " (since " + moment(task.begin).fromNow(true).blue + ")");
		} else {
			console.log("Nothing. " + what.green + " finished " + moment(task.end).fromNow().blue);
		}
	} else {
		console.log("Nothing.");
	}
...
punch.out = punchCommand(function(options) {
	...
	if (task && task.isActive()) {
		...
		console.error("No task to end".red);
		process.exit(1);
	}
});
